...
public class EventCenter
{
    ...
    public static void AddListener(EventDefine eventType, CallBack callBack)
    {
        ...
        m_EventTable[eventType] = (CallBack)m_EventTable[eventType] + callBack;//+号前的部分不写对这个应用不影响
        //因为这个应用每个事件码都只用到了一次，即每个EvenDefine只用了一次AddListener,如果需要让一个事件码控制两
        //个事件则需要，否则实现的将会是最后一个赋值的方法。
    }
    //Single parameters
